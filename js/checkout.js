...
form.addEventListener('submit',async event=>{
  ...
  try{
    ...
    if(activeMode==='stripe'){
      ...
      const response=await fetch('api/create-checkout-session.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({orderId})});const payload=await response.json().catch(()=>({}));if(!response.ok||!payload.url)throw new Error(payload.error||'STRIPE_SESSION_FAILED');const metaItems=orderItems.map(item=>({id:String(item.code||item.id),quantity:Number(item.quantity)||1,item_price:Number(item.price)||0}));window.CasaGlickMetaPixel?.trackOnce?.(`checkout:${String(payload.sessionId||orderId)}`,'InitiateCheckout',{content_ids:metaItems.map(item=>item.id),contents:metaItems,content_type:'product',num_items:metaItems.reduce((sum,item)=>sum+item.quantity,0),value:Number(total)||0,currency:'MXN'});sessionStorage.setItem(STRIPE_PENDING_KEY,JSON.stringify({orderId,folio:activeFolio,signature,sessionId:String(payload.sessionId||'')}));window.location.assign(payload.url);return;
    }
    ...
    const metaItems=orderItems.map(item=>({id:String(item.code||item.id),quantity:Number(item.quantity)||1,item_price:Number(item.price)||0}));
    window.CasaGlickMetaPixel?.trackOnce?.(`assisted-checkout:${orderRef.id}`,'InitiateCheckout',{content_ids:metaItems.map(item=>item.id),contents:metaItems,content_type:'product',num_items:metaItems.reduce((sum,item)=>sum+item.quantity,0),value:Number(total)||0,currency:'MXN'});
    window.CasaGlickMetaPixel?.trackOnce?.(`assisted-lead:${orderRef.id}`,'Lead',{content_name:'Solicitud de compra asistida',content_category:'Compra asistida',value:Number(total)||0,currency:'MXN'});
