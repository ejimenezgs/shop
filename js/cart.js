(function(){
  ...
  const add=product=>{const items=read();const id=String(product.id||product.code);const found=items.find(item=>String(item.id)===id);const max=Math.max(1,Number(product.stock)||1);if(found)found.quantity=Math.min(max,(Number(found.quantity)||1)+1);else items.push({id,code:product.code,name:product.name,price:Number(product.price),image:product.image,stock:max,quantity:1});write(items);const price=Number(product.price);window.CasaGlickMetaPixel?.track?.('AddToCart',{content_ids:[String(product.code||product.id||id)],content_name:String(product.name||product.code||'Producto'),content_type:'product',value:Number.isFinite(price)?price:0,currency:'MXN'});return items};
